...
struct Trie {
    ...
    int check(const char* key, int prev, int press) {
        ...
        int i = *key-'a';
        if (prev == next[i]->cnt) return next[i]->check(key+1,prev,press);
        else return next[i]->check(key+1,next[i]->cnt, press+1);
    }
};
...
int main(){    
    ...
    while(scanf("%d",&n) != -1) {
        ...
        int ans = 0;
        for (int i=0; i<n; i++) ans += root->check(vc[i].c_str(),0,0);
        printf("%.2lf\n",double(ans)/n);
        delete root;
    }
}